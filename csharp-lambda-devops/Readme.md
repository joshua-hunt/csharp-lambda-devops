...
WORKDIR /src
COPY ["csharp-lambda-devops.csproj", "csharp-lambda-devops/"]
RUN dotnet restore "csharp-lambda-devops/csharp-lambda-devops.csproj"

WORKDIR "/src/csharp-lambda-devops"
COPY . .
RUN dotnet build "csharp-lambda-devops.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "csharp-lambda-devops.csproj" \
...
Execute unit tests
```
cd "csharp-lambda-devops/test/csharp-lambda-devops.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "csharp-lambda-devops/src/csharp-lambda-devops"
